...
def request_api(event, context):
    ...
    # Get API config
    db = firestore.Client()
    api_config_ref = db.collection(u'tentacles').document(u'ApiConfig').collection(config['api']).document(
        config['config'])
...
def get_userlist_id_from_name(user_list_service, list_name):
...
def send_data_customer_match(records, messageId, config):
    ...
    user_list_id = get_userlist_id_from_name(user_list_service, list_name)

    members = [json.loads(email) for email in hashed_emails]
    members_add = [{'hashedEmail': member['hashedEmail']} for member in members if member['operator'] == 'ADD']
    members_remove = [{'hashedEmail': member['hashedEmail']} for member in members if member['operator'] == 'REMOVE']

    mutate_members_add_operation = {
        'operand': {
            'userListId': user_list_id,
            'membersList': members_add
        },
        'operator': 'ADD'
    }

    mutate_members_remove_operation = {
        'operand': {
            'userListId': user_list_id,
            'membersList': members_remove
        },
        'operator': 'REMOVE'
    }

    mutate_members_operations = []
    if len(members_add) > 0:
        mutate_members_operations.append(mutate_members_add_operation)
    if len(members_remove) > 0:
        mutate_members_operations.append(mutate_members_remove_operation)

    response = user_list_service.mutateMembers(mutate_members_operations)

    if 'userLists' in response:
        for user_list in response['userLists']:
            print('User list with name "%s" and ID "%d" was updated.'
